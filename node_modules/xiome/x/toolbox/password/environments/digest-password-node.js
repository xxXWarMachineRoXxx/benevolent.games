...
export async function digestPasswordNode({ salt, password }) {
    ...
    const decode = (subject) => Buffer.from(decodeHex(subject));
    const buffer = await pbkdf2(decode(password), decode(salt), digestIterations, digestByteLength, digestAlgorithm);
    const digest = buffer.toString("hex");
    return digest;
}
//# sourceMappingURL=digest-password-node.js.map