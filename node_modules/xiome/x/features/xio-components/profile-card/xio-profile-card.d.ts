import { User } from "../../auth/aspects/users/types/user.js";
import { AutowatcherComponent } from "../../../framework/component.js";
import { ProfileDraft } from "../../auth/aspects/users/routines/personal/types/profile-draft.js";
export declare class XioProfileCard extends AutowatcherComponent {
    #private;
    init(): void;
    ["show-details"]: boolean;
    user?: User;
    saveProfile?: (profile: ProfileDraft) => Promise<void>;
    private profileDraft;
    private get draftIsChanged();
    private get readonly();
    private problems;
    private getTextInputField;
    private generateNewProfileDraftFromInputs;
    private handleChange;
    private handleSave;
    render(): import("lit-html").TemplateResult<1>;
}
